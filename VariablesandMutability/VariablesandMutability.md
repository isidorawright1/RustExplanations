...
2. [Declaring Constants](#Declaring-Constants)
...
By default, in Rust, variables are **immutable**, which means the value of a viable cannot change throughout the code.
...
The reason Rust makes variable immutable by default is for safety. Code is prone to human error and attacks. By making a variable unchangeable by default, it ensures that a it can't be changed on accident by fat-fingering a number key and an attacker can't change the value of a variable easily.
...
Constants can be defined in any scope needed. Constants are mainly used for a value that multiple parts of the code may need to know.
...
This can be done by using the same variable name with the keyword ***let*** again. Since the keyword ***let*** is being used, it is like you are defining a new variable but with the same name. This means that the new variable can have a different type than the previous one if desired.
